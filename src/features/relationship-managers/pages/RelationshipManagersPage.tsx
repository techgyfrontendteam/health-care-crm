...
export const RelationshipManagersPage: React.FC<RelationshipManagersPageProps> = ({ lockedRmId }) => {
  ...
  const { data: liveRmUsers = [] } = useGetAllUsersByRoleIdQuery({
    role_id: 3,
    offset: 0,
  });
  ...
  const { data: liveEmUsers = [] } = useGetAllUsersByRoleIdQuery({
    role_id: 4,
    offset: 0,
  });
  ...
  const rmData = useMemo(() => {
    if (!rmDashboardApiResponse || Object.keys(rmDashboardApiResponse).length === 0) {
      return getRMDashboardData(activeProjectLabel);
    }
  ...
  }, [activeProjectLabel, rmDashboardApiResponse, staleResponse, escalatedResponse, selectedRmId]);
  ...
  if (view === "dashboard" && isApiLoading && rmUsers.length === 0) {
    return (
  ...
  if (view === "dashboard" && apiError && rmUsers.length === 0) {
    return (
  ...
  return (
    <div className="w-full max-w-[1440px] xl:max-w-[1920px] 2xl:max-w-[2560px] mx-auto space-y-4 px-3 sm:px-5 md:px-6 py-4 animate-in fade-in duration-300">
              ...
              <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button
                      className="flex items-center justify-between w-40 bg-[#f8fafc] dark:bg-zinc-850 hover:bg-slate-100 dark:hover:bg-zinc-800 border border-slate-200/50 dark:border-zinc-800 px-3.5 py-2 rounded-xl text-xs font-bold text-slate-700 dark:text-zinc-300 transition-colors shadow-sm cursor-pointer gap-2"
                    >
                      <span className="truncate">{selectedRmLabel === "All" ? "All Sales Executives" : selectedRmLabel}</span>
                      <ChevronDown className="w-3.5 h-3.5 text-slate-500 shrink-0 ml-1.5" />
                    </button>
                  </DropdownMenuTrigger>
      ...
      */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
        {/* Team Calls Summary */}
        <div className="dashboard-focus-card flex min-h-[170px] flex-col justify-between rounded-md border border-[#0F1A34] bg-[#0F1A34] p-4.5 text-white shadow-sm transition-all duration-200 hover:shadow-md">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-extrabold uppercase tracking-wider text-white/80">
              Team Calls Summary
            </h2>
            <span className="rounded-md bg-white/10 px-2 py-0.5 text-[10px] font-bold text-white/80">
              All Channels
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 mt-1">
            <div className="rounded-md border border-white/10 bg-white/10 p-2.5 text-left backdrop-blur-md">
              <span className="block text-[9px] font-bold uppercase tracking-wider text-white/80">Connected</span>
              <span className="text-lg font-black mt-0.5 block">
                {rmData.teamCallsSummary.connectedCalls - rmData.teamCallsSummary.missedCalls}
              </span>
            </div>
            <div className="rounded-md border border-white/10 bg-white/10 p-2.5 text-left backdrop-blur-md">
              <span className="block text-[9px] font-bold uppercase tracking-wider text-white/80">Missed</span>
              <span className="mt-0.5 block text-lg font-black text-white">{rmData.teamCallsSummary.missedCalls}</span>
            </div>
          </div>
        </div>
        ...
        <DialogContent className="max-w-[620px] overflow-hidden flex flex-col p-6 gap-5 border-none shadow-3xl bg-white dark:bg-zinc-950 rounded-[28px]">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-200/60 bg-slate-50 py-3 pl-11 pr-4 text-sm font-semibold text-slate-700 shadow-xs transition-all placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0f3d6b] dark:border-zinc-800/80 dark:bg-zinc-900 dark:text-zinc-355 dark:placeholder-zinc-500 dark:focus:ring-blue-500"
              value={popupSearch}
